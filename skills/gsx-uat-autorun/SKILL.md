...
> ⚠️ **Chinese curl — prevent UTF-8 mojibake** (shell-inline Chinese JSON double-encodes and stores garbage): `Write` the body to a UTF-8 temp file, then `curl --data @/tmp/xxx.json -H 'Content-Type: application/json; charset=UTF-8'`. Read back via list/detail API, assert Chinese fields char-by-char; any `Ã / Â / å / æ / ç` → mojibake → **stop and report**, don't bulk-write corrupt data.
